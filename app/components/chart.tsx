  ...
  useEffect(() => {
    async function fetchData(address: string) {
      try {
        ...
        const response_geoIdV4 = await axios.get(
          'https://api.gateway.attomdata.com/v4/area/hierarchy/lookup?WKTString=POINT(' +
        ...
        for (const key in response_geoIdV4.data.response.result.package.item) {
          const currentItem =
            response_geoIdV4.data.response.result.package.item[key]
        ...
        const options = {
          method: 'GET',
          url:
            'https://api.gateway.attomdata.com/v4/transaction/salestrend?geoIdV4=' +
            geoIdV4 +
            '&interval=monthly&startyear=2020&endyear=2024',
          headers: { apikey: process.env.ATTOM_API_KEY },
        }
        ...
        const extractedData = response_final.data.salesTrends.map(
          (sale: any) => {
            return {
